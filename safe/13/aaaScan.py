...
class PortScanner:
    ...
    def radiuscrack(self,localsocket, ip, port_number):
        ...
        pkt_len = avp_pwd_len + avp_uname_len + len(authenticator) + len(pack_id) + len(radius_code) + 2
        ...
        if ready[0]:
            ...
            if resp_code == '01' or resp_code == '02' or resp_code == '03':  # radius service is on
                content = "***************found radius service *******************" + str(port_number)
                print(content)
                try:
    ...
    def scan(self, server_ip, message=''):
        ...
        print('server {} scanned in  {} seconds'.format(server_ip, stop_time - start_time))
        print('scan finished !\n')
    ...
    def __scan_ports_helper(self, ip, delay, tcpoutput, udpoutput, message):
        ...
        while port_index < self._portnum:
            ...
            while threading.activeCount() < self.__thread_limit and port_index < self._portnum:
                # Start threads
                if int(self._type) == 1:
                    thread = threading.Thread(target=self.__TCP_connect,
                                              args=(ip, self._beginport + port_index, delay, tcpoutput, message,self.tacacsfile))
                else:
                    thread = threading.Thread(target=self.__UDP_connect,
                                              args=(
                                                  ip, self._beginport + port_index, delay, udpoutput, message,self.radiusfile))
                thread.start()
    ...
    def __scan_ports(self, ip, delay, message):
        ...
        thread = threading.Thread(target=self.__scan_ports_helper, args=(ip, delay, tcpoutput, udpoutput, message))
        thread.start()
    ...
    def tacacscrack(self,ip, port_number,tcpoutput):
        try:
            ...
            cli = TACACSClient(ip, port_number, secretkey, timeout=10, family=socket.AF_INET)
            authen = cli.authenticate(username, password)
            print(authen.valid)
            if True == authen.valid or False == authen.valid:
                print("***************found tacacs+ service *******************" + str(port_number))
                content = "***************found tacacs+ service ******************* {0}".format(str(port_number))
    ...
    def __UDP_connect(self, ip, port_number, delay, udpoutput, message,radiusfile):
        ...
        try:
            ...
            hosts_list = [(port_number, nm[x][u'udp'][port_number]['state']) for x in nm.all_hosts()]
...
def main():
    ...
    parser.add_argument('-t', '--type', dest="type", type=int,help="type", required=True,default=1)
    parser.add_argument('-H', '--host', dest="host", help="hostip", required=True)
    parser.add_argument('-b', '--beginport',type=int, dest="beginport", help="beginport", required=True)
    parser.add_argument('-n', '--portnum', type=int,dest="portnum", help="portnum", required=True)
